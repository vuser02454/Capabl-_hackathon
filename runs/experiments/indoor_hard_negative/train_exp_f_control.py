...
def main():
    ...
    data_yaml = repo_root / "runs/experiments/indoor_hard_negative/dataset_control/data.yaml"
    weights = repo_root / "backend/models/yolov8n.pt"
    project_dir = repo_root / "runs/experiments/indoor_hard_negative"
    name = "exp_f_yolov8n_control"

    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print(f"Starting Experiment F CONTROL Training: device={device}, data={data_yaml}, base={weights}")
    ...
    if best.is_file():
        import shutil
        shutil.copy2(best, ckpt_dir / "best_exp_f.pt")
        print(f"Saved best checkpoint to {ckpt_dir / 'best_exp_f.pt'}")
    if last.is_file():
        import shutil
        shutil.copy2(last, ckpt_dir / "last_exp_f.pt")
        print(f"Saved last checkpoint to {ckpt_dir / 'last_exp_f.pt'}")

    manifest = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "purpose": "EXPERIMENT F (CONTROL) - TACO only, NO indoor hard negatives",
        ...
        "saved_checkpoints": [
            str(ckpt_dir / "best_exp_f.pt"),
            str(ckpt_dir / "last_exp_f.pt")
        ],
        "metrics_summary": {
    ...
    with open(project_dir / "training_manifest_exp_f.json", "w") as f:
